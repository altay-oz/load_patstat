...
if psql -lqt | cut -d \| -f 1 | grep -qw $PATSTAT_DB_NAME; then
    # database exists
    echo "$PATSTAT_DB_NAME database exists, installation starts"
else
    echo "$PATSTAT_DB_NAME database doesn't exist."
...
do
   ...
   do
       ...
       psql -c "\COPY $table_name from '$TMP_DIR/file_to_be_inserted.csv' DELIMITER AS ',' CSV HEADER QUOTE AS '\"' " $PATSTAT_DB_NAME
       echo "INSERTED $file"
       echo " "
   done
done
...
if [ $INDEX_Y == 'y' ]; then
    ...
    psql $PATSTAT_DB_NAME < ./create_patstat_keys.sql
else
